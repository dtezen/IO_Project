...
print("                    Método de Esquina NorOeste")
print(" ")
print(" ")
...
def getValores(origenes, destinos):
    ...
    for x in range(origenes):
        for y in range(destinos):
            valores[x][y] = verifyIntInput(f'Introducir el valor del origen {x + 1} con el destino {y + 1}\n')
    return valores

def getOferta(num):
    oferta = []
    for x in range(num):
        aux = verifyIntInput(f'Introducir la oferta del origen {x + 1}\n')
        oferta.append(aux)
    return oferta
...
def getDemandas(num):
    demanda = []
    for x in range(num):
        aux = verifyIntInput(f'Introducir la demanda del destino {x + 1}\n')
        demanda.append(aux)
    return demanda
